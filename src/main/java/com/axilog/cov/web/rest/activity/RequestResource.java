...
/**
 * REST controller for managing {@link io.github.jhipster.sample.domain.activity.Request}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class RequestResource {
    ...
    public ResponseEntity<Request> createRequest(@RequestBody Request request) throws URISyntaxException {
        ...
        //request.setStatus(RequestStatus.builder().status(RequestStatusEnum.CREATED).updatedAt(DateUtil.convertToLocalDateViaInstant(DateUtil.now())).build());
        Request result = requestRepository.save(request);
        log.info("Starting new workflow for equipment request for: {}", result.getRequestId());
    ...
    public ResponseEntity<Void> deleteRequest(@PathVariable Long id) {
        ...
        if (request.isPresent()) {
        	//request.get().getStatus().setStatus(RequestStatusEnum.CANCELED.getLabel());
        }
